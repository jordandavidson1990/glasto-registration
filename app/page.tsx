  ...
  const people = useMemo(() => {
    ...
    return term
      ? registrations.filter((person) =>
          Object.values(person).join(" ").toLowerCase().includes(term)
        )
      : registrations;
  }, [query]);
    ...
    <main>
      <header className="site-header">
        <a
          className="site-name"
          href="#top"
          aria-label="Glastonbury registrations"
        >
          <Image
            src="/Glastonbury%20-%20Registration%20Lookup_files/glastonbury-banner-2027.png"
      ...
      <section
        className="stage"
        id="top"
        onMouseMove={shiftArtwork}
        onMouseLeave={() => setImagePosition({ x: 50, y: 45 })}
      >
        <Image
          className="stage-image"
          src="/pyramid-stage-art.gif"
          alt="The Pyramid Stage at Glastonbury Festival"
          fill
          priority
          unoptimized
      ...
      <section className="registry" aria-labelledby="registry-title">
        <div className="registry-heading">
          ...
          <label className="search">
            <span className="visually-hidden">Search registrations</span>
            <input
              value={query}
        ...
        <div className="table" role="table">
          <div className="table-labels" role="row">
            <span>NAME</span>
          ...
          {people.map((person, index) => (
            <div className="person-row" role="row" key={person.id}>
              ...
              <button onClick={() => copyDetails(person)}>
      ...
      <footer>A JDawg Production</footer>
    </main>
  );
}
